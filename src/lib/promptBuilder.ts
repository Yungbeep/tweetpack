...
export function buildClaudePrompt(
  ...
  const successfulLinks = links.filter((l) => !l.error);

  let prompt = `# Claude Code Prompt — Build from Tweet Pack

## Context

This build pack was generated from a tweet by @${tweet.handle || "unknown"}.

Original tweet: ${tweet.canonicalUrl}
...
`;
  ...
  if (successfulLinks.length > 0) {
    ...
    for (const link of successfulLinks) {
      prompt += `- \`sources/${link.filename}\` — ${link.title || link.url}\n`;
    }
  }
